...
function init(mode) {
  ...
  // prettier-ignore
  my.clrs = [["PaleGreen", '#98FB98'], ["SpringGreen", '#00FF7F'], ["Thistle", '#D8BFD8'],  ["Yellow", '#FFFF00'], ["Gold", '#FFD700'],["Pink", '#FFC0CB'], ["LightSalmon", '#FFA07A'], ["Lime", '#00FF00'], ["DarkSeaGreen", '#8FBC8F'], ["Orange", '#FFA500'], ["Khaki", '#F0E68C'], ["Violet", '#EE82EE'], ["Teal", '#008080'], ["LightBlue", '#ADD8E6'], ["SkyBlue", '#87CEEB'], ["Blue", '#0000FF'], ["Navy", '#000080'], ["Purple", '#800080'], ["Wheat", '#F5DEB3'], ["Tan", '#D2B48C'], ["AntiqueWhite", ["SlateBlue", '#6A5ACD'], '#FAEBD7'], ["Aquamarine", '#7FFFD4'], ["Silver", '#C0C0C0']];
  ...
  s += '<div style="position:relative; width:' + w + 'px; height:' + h + 'px;  margin:auto; display:block; border: 1px solid var(--shade);  border-radius: 10px; 		box-shadow: 5px 5px 10px var(--shade); ">'

  s += '<div id="btns0" style="position:absolute; left:5px; top:3px;">'
  s += '<span style="font: 20px Arial; text-align: center; ">Disks: </span>'
  //s += '<div id="num" style="display: inline-block; text-align: center; padding: 2px 20px 2px 20px; border-radius: 10px; font: 24px Arial; background-color: #00bfff ">3</div>'
...
function successDo() {
  document.getElementById('success').innerHTML = 'Well Done !'
...
function hanoi(from, to, buf, nmv) {
  //console.log("hanoi",from, to, buf, nmv);
  if (nmv > 1) {
...
class Mouse {
  ...
  onMouseDown(ev) {
    ...
    if (my.drag.n >= 0) {
      console.log('drrragin!', my.drag.n)
  ...
  onMouseUp() {
    if (my.drag.onQ) {
      my.shapes[my.drag.n].div.style.filter = 'none'
      my.drag.onQ = false
    }
    document.body.style.cursor = 'default'
  }
  ...
  hitTest(shape, mouse) {
    if (mouse.x < shape.x) return false
